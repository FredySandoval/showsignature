...
Options:
  ...
  --lang-only <lang>     only process files for the provided language
                         (optional) inferred from file extension if not
                         provided
                         supported extensions: .cjs, .cts, .js, .md, .mjs,
                         .mts, .py, .ts
  --show-only <options>  comma-separated extract kinds to include
                         code: signatures, interfaces, types, variables,
                         comments, imports
                         markdown: md:all, md:caveman, md:codeblocks,
                         md:headings, md:tables (legacy alias: md -> md:all)
                         default: signatures --line-number
  --file <file>          process a single file
  --folder <folder>      process files from a folder (.gitignore files are
                         respected)
