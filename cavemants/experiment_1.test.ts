...
describe("toCaveman", () => {
  ...
  test("splits because into fragments", () => {
    expect(
      toCaveman("The frontend is slow because it renders all tasks at once."),
    ).toBe("frontend is slow. it renders all tasks at once");
  });
  ...
  test("keeps technical tokens", () => {
    ...
    ).toBe("run `npm run test` in /src/app.ts with NODE_ENV=v1.2.3");
  });
  ...
  test("does not protect normal quoted text", () => {
    expect(
      toCaveman('The label is "the simple guide" and it is basically fine.'),
    ).toBe('label is "simple guide" and it is fine');
  });

  test("ultra mode uses arrow", () => {
    expect(
      toCaveman("New object reference causes re-render.", { ultra: true }),
    ).toBe("New object reference → re-render");
  });
  ...
  test("compresses blockquotes without breaking markdown marker", () => {
    expect(
      toCaveman("> The API is basically slow because it renders everything."),
    ).toBe("> API is slow. it renders everything");
  });

  test("removes intensifiers and extra hedging", () => {
    expect(toCaveman("The API is very slow and probably unstable.")).toBe(
      "API is slow and unstable",
    );
  });
  ...
  test("rewrites verbose phrases", () => {
    expect(
      toCaveman("In the event that the build fails, restart service."),
    ).toBe("if build fails, restart service");
  });

  test("rewrites due-to-the-fact-that phrases", () => {
    expect(
      toCaveman("Due to the fact that the cache is stale, reload data."),
    ).toBe("cache is stale, reload data");
  });

  test("rewrites ability phrases", () => {
    expect(
      toCaveman("Service is able to retry and has the ability to recover."),
    ).toBe("Service can retry and can recover");
  });

  test("drops weak determiners in common phrases", () => {
    expect(toCaveman("This issue happens after deploy.")).toBe(
      "issue happens after deploy",
    );
    expect(toCaveman("This problem blocks deploy.")).toBe(
      "problem blocks deploy",
    );
    expect(toCaveman("This change fixes build.")).toBe("change fixes build");
  });

  test("removes support-chat politeness fluff", () => {
    expect(toCaveman("Thanks, I am happy to help. Hope that helps.")).toBe(
      "I am help",
    );
  });
  ...
  test("rewrites negation phrases", () => {
    expect(
      toCaveman("Client does not have token and service is not able to retry."),
    ).toBe("Client lacks token and service cannot retry");
  });
  ...
  test("ultra mode rewrites more causal phrases", () => {
    ...
    ).toBe("State update → re-render cache miss → retry");
  });
});
