...
async function writeFixtureFile(rootDir: string, relativePath: string): Promise<string> {
...
describe('discoverFiles', () => {
  test('discovers recursively and excludes test files', async () => {
    ...
    const registry = createLanguageRegistry();
    registry.register(createMockAdapter({ id: 'ts', extensions: ['.ts', '.tsx'] }));
  ...
  test('can include test files when explicitly requested', async () => {
    ...
    const fromNamedTest = await writeFixtureFile(rootDir, 'src/component.test.ts');
  ...
  test('returns supported explicit file even if it matches test naming', async () => {
    ...
    const explicitFile = await writeFixtureFile(rootDir, 'src/component.test.ts');

    await expect(discoverFiles({ registry, file: explicitFile })).resolves.toEqual([
      explicitFile,
    ]);
  });
  ...
  test('returns empty list for explicit file with unsupported extension', async () => {
    ...
    await expect(discoverFiles({ registry, file: unsupportedFile })).resolves.toEqual(
      [],
    );
  });
