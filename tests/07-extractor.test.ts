...
function createTestContext(overrides?: Partial<TestParseContext>): TestParseContext {
    return {
        source: '',
...
describe('08-extractor — runExtractors', () => {
    ...
    describe('combined output', () => {
        test('merges and sorts entries by source position', () => {
            const adapter = createTestAdapter([
                ...
                createStubExtractor('signatures', ['function run(): void;'], { sourcePos: 3 }),
            ]);
        ...
        test('strips positional metadata from combined output entries', () => {
            const adapter = createTestAdapter([
                createStubExtractor('imports', ['import fs from "node:fs";'], { sourcePos: 0 }),
            ]);
            const context = createTestContext();
        ...
        test('uses FALLBACK position for entries without sourcePos', () => {
            const adapter = createTestAdapter([
                ...
                createStubExtractor('signatures', ['function early(): void;'], { sourcePos: 1 }),
            ]);
            const context = createTestContext();
    ...
    describe('unsupported kind warnings', () => {
        test('emits a warning for unsupported extract kinds', () => {
            const adapter = createTestAdapter([
                createStubExtractor('signatures', ['function run(): void;'], { sourcePos: 0 }),
            ]);
        ...
        test('mixes supported results with unsupported warnings', () => {
            const adapter = createTestAdapter([
                createStubExtractor('imports', ['import x from "y";'], { sourcePos: 0 }),
            ]);
            const context = createTestContext();
            ...
            expect(result.warnings.map((w) => w.kind)).toEqual(['variables', 'types']);
        });
    });
    ...
    describe('edge cases', () => {
        ...
        test('handles single kind in extractOrder', () => {
            const adapter = createTestAdapter([
                createStubExtractor('interfaces', ['interface Foo {}'], { sourcePos: 0 }),
            ]);
            const context = createTestContext();
        ...
        test('always returns entries in source order', () => {
            const adapter = createTestAdapter([
                ...
                createStubExtractor('signatures', ['function pos5(): void;'], { sourcePos: 5 }),
            ]);
            const context = createTestContext();
