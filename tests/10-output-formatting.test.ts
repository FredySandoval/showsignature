...
function makeSection(overrides: Partial<FileSection> & { filePath: string }): FileSection {
    return {
        lang: 'ts',
        entries: [],
        warnings: [],
...
describe('detectFenceLanguage', () => {
    test('returns adapter fenceLang when explicitLang matches a registered adapter', () => {
        ...
        registry.register(
            createMockAdapter({ id: 'ts', extensions: ['.ts'], fenceLang: 'typescript' }),
        );
